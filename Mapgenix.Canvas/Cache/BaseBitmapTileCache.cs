    ...
    public abstract class BaseBitmapTileCache : BaseTileCache
    {
       ...
       /// <returns>Returns a collection of BitmapTiles based on a bounding box passed in.</returns>
        ...
        protected BitmapTile MergeBitmaps(IEnumerable<BitmapTile> tilesToMerge, double scale)
        {
            ...
            var sourceRectangle = new Rectangle(0, 0, entireWidth, entireHeight);
            var mergedBitmap = new Bitmap(sourceRectangle.Width, sourceRectangle.Height);
            Graphics graphics = null;

            try
            {
                graphics = Graphics.FromImage(mergedBitmap);
                ...
                foreach (var bitmapTile in tilesToMerge)
                {
                    ...
                    {
                        graphics.DrawImageUnscaled(bitmapTile.Bitmap, (int) Math.Round(drawingUpperLeftX),
                            (int) Math.Round(drawingUpperLeftY));
                    }
                }
            }
            finally
