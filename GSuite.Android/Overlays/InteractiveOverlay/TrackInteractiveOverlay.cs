       ...
       public int MouseDownCount
        ...
        private void EndTracking()
        {
            ...
            if (tempFeature != null)
            {
                ...
                if(features.Count > 0)
                {
                    _trackShapeLayer.FeatureSource.BeginTransaction();

                    try
                    {
                        foreach (Feature f in features)
                        {
                            _trackShapeLayer.FeatureSource.DeleteFeature(f.Id);
                        }
                        _trackShapeLayer.FeatureSource.CommitTransaction();
                    }
                    catch (Exception)
                    {
                        _trackShapeLayer.FeatureSource.RollbackTransaction();
                        throw;
                    }
                }
            }
