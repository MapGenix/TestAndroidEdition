...
namespace Mapgenix.GSuite.Android
{
    ...
    internal class OpenStreetMapAndroidLayer : BaseLayer
    {
        ...
        public OpenStreetMapAndroidLayer()
            : this(null)
        {
        }

        public OpenStreetMapAndroidLayer(WebProxy webProxy)
        {
        ...
        public OpenStreetMapAndroidLayer(WebProxy webProxy, string cacheDirectory)
        {
            //imageCache = new OpenStreetMapLayerImageCache();
        ...
        private void FetchImageInThread(object obj)
        {
            ...
            try
            {
                using (Stream imageStream = request.GetResponse().GetResponseStream())
                {
                    ...
                    if (image == null)
                    {
                        image = Bitmap.CreateBitmap(tileWidth, tileHeight, Bitmap.Config.Argb8888);
                        Graphics exceptionGraphic = Graphics.FromImage(image);
                        exceptionGraphic.FillRectangle(Color.Aquamarine, 0, 0, tileWidth, tileHeight);
                    }
            ...
            catch (Exception ex)
            {
                ...
                Bitmap sourceBitmap = Bitmap.CreateBitmap(tileWidth, tileHeight, Bitmap.Config.Argb8888);
                Graphics exceptionGraphic = Graphics.FromImage(sourceBitmap);
                exceptionGraphic.FillRectangle(Color.Aquamarine, 0, 0, tileWidth, tileHeight);

                images.TryAdd(imageId, sourceBitmap);
